...
int main(void){
	...
	while(1) {
		...
		//The condition below is to check if the BUTTON is PRESSED
		if(!(GPIOC->IDR & GPIO_IDR_ID13) == 0){

			//turn ON PC5, PC6, PC8, and PC9 (which is the External LED)
			GPIOC->BSRR |= GPIO_BSRR_BS5 | GPIO_BSRR_BS6 | GPIO_BSRR_BS8 | GPIO_BSRR_BS9;

			//turn OFF PA5 (which is the Internal LED)
			GPIOA->BSRR |= GPIO_BSRR_BR5;

		}else{

			//Turn OFF PC5,PC6, PC8 and PC9 (external LED)
			GPIOC->BSRR |= GPIO_BSRR_BR5 | GPIO_BSRR_BR6 | GPIO_BSRR_BR8 | GPIO_BSRR_BR9;


			//Turn ON pa5 (internal LED)
